...
export default function WatermarkPage() {
  const [activeType, setActiveType] = useState('dynamic'); // dynamic | static
  ...
  const [textColor, setTextColor] = useState('#64748B'); // slate-500
  const [textOpacity, setTextOpacity] = useState(25); // percentage
  const [rotation, setRotation] = useState(-30);

  // Active positions grid map (3x3 grid)
  ...
  // Build the dynamic preview text string
  ...
  // Convert Hex color to RGBA for watermark styling
    ...
    <div className="relative min-h-screen bg-[#F8FAFC]">
      ...
      <div className="relative p-4 md:p-6 max-w-5xl mx-auto w-full space-y-6 animate-in slide-in-from-bottom-4 fade-in duration-700">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <button className="px-6 py-2.5 bg-gradient-to-r from-gray-900 to-black text-white text-sm font-medium rounded-xl hover:shadow-lg hover:shadow-gray-900/20 hover:-translate-y-0.5 transition-all duration-300">
            Save Changes
          </button>
        </div>

        {/* Live Preview & Settings Panel Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">

          {/* Left panel: Form Settings (7 cols) */}
          <div className="lg:col-span-7 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <button
                          ...
                          className={`flex items-center justify-between p-3 rounded-xl border text-[13px] font-semibold transition-all ${attributes[item.id]
            ...
            {/* Visual Position Selection Card */}
            <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-3 gap-2 bg-gray-50 p-3 rounded-2xl border border-gray-100 max-w-sm mx-auto">
                {Object.keys(positions).map((pos) => {
                  const label = pos.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                  return (
                    <button
                      key={pos}
                      onClick={() => handlePositionToggle(pos)}
                      className={`h-16 rounded-xl flex flex-col items-center justify-center gap-1.5 text-[11px] font-bold border transition-all ${positions[pos]
          ...
          {/* Right panel: LIVE PREVIEW mockup (5 cols) */}
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-200 p-6 flex-1 flex flex-col">
              ...
              {/* Simulated PDF / Doc Preview Canvas */}
              <div className="relative flex-1 min-h-[420px] bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden flex flex-col p-4 shadow-inner">
                ...
                {/* Simulated Document Content Layer */}
