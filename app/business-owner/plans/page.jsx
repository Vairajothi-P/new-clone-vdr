...
export default function BusinessOwnerPlansPage() {
  ...
  const handleDeletePlan = async (plan) => {
    if (plan.name === 'Free' || plan.name === 'Pro' || plan.name === 'Enterprise') {
      alert('System default plans cannot be deleted.');
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white text-sm font-medium rounded-xl hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2"
          >
            <FaPlus />
        ...
        {isLoading ? (
          ...
          plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className="bg-white/80 backdrop-blur-xl border border-slate-200/80 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-slate-300 transition-all flex flex-col justify-between group"
              >
                <div>
                  {/* Top Badge & Price */}
