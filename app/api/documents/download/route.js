    ...
    try {
        ...
        if (actionType === 'original') {
            ...
            if (fileExt !== 'pdf' && watermarkSettings) {
                throw new Error("Native watermarking for original non-PDF files is not currently supported. Please use Secure Download instead.");
            }
        ...
        // 3. Handle SECURE HTML Download
            ...
            const htmlContent = generateSecureHtmlWrapper(doc.id, doc.name, fileExt, encryptedPayload, backendUrl, watermarkSettings, brandLogo);
