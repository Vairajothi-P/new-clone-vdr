...
export default function TaskBoard({ tasks, allTasks, onTaskClick, viewMode }) {
  ...
  return (
    <div className="h-full flex flex-col space-y-4 animate-in fade-in duration-300">
      ...
      {/* Sleek Filters */}
      <div className="flex flex-wrap md:flex-nowrap gap-3 md:gap-4 border-b border-slate-200/60 pb-4">
        <select
          value={filterRole}
          onChange={(e) => setFilterRole(e.target.value)}
          className="flex-1 min-w-[140px] text-sm font-semibold text-slate-700 bg-white border border-slate-200/80 rounded-xl py-2 px-4 focus:ring-2 focus:ring-[var(--brand,theme(colors.blue.500))] focus:border-transparent outline-none shadow-sm hover:bg-slate-50 transition-colors"
        >
      ...
      <div className="flex xl:grid xl:grid-cols-5 gap-4 pb-4 px-1 flex-1 items-start pt-2 overflow-x-auto snap-x [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {columns.map(col => {
          ...
          return (
            <div key={col.id} className="w-[85vw] sm:w-[320px] xl:w-auto shrink-0 bg-gradient-to-b from-[var(--brand-50,theme(colors.blue.50))] to-[var(--brand-50,theme(colors.slate.50))]/20 rounded-2xl p-4 flex flex-col max-h-full border border-[var(--brand-100,theme(colors.blue.100))] shadow-[0_4px_20px_-10px_rgba(var(--brand-rgb),0.15)] snap-center">
              ...
              <div className="space-y-3 overflow-y-auto px-1 pb-2 flex-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {colTasks.map(task => {
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white/95 backdrop-blur-sm border rounded-xl p-5 shadow-sm hover:shadow-md hover:-translate-y-1 cursor-pointer transition-all duration-300 group relative flex flex-col min-h-[160px] ${isOverdue ? 'border-red-200 bg-red-50/20 hover:border-red-300' : 'border-[var(--brand-100,theme(colors.slate.200))] hover:border-[var(--brand,theme(colors.blue.400))] hover:shadow-[0_4px_12px_-4px_rgba(var(--brand-rgb),0.2)]'}`}
                    >
